...
/*
 * This code came from Cell0907 as an exercise in programming.  At this 
 * stage all that has been done to it is to subsitute Core.NATIVE_LIBRARY_NAME 
 * for the actual name of the OpenCV library.  This and the recognition of 
 * the 7" strips of reflective tape will constitute 4949's  introduction to
 * vision processing in 2015.  We will see how it goes.
 */
...
public class FindRectangles {  
  public static void main(String arg[]){  
    ...
    RectPanel panel1 = new RectPanel();  
    ...
    RectPanel panel2 = new RectPanel();  
    ...
    RectPanel panel3 = new RectPanel();  
    ...
    RectPanel panel4 = new RectPanel();  
    ...
    Mat rectangles = new Mat(); // No need (and don't know how) to initialize it.  
     ...
     {  
       ...
       if( !webcam_image.empty() )  
        {  
         ...
         // Apply the Hough Transform to find the rectangles  
         Imgproc.GaussianBlur(thresholded, thresholded, new Size(9,9),0,0);  
         Imgproc.HoughCircles(thresholded, rectangles, Imgproc.CV_HOUGH_GRADIENT, 2, thresholded.height()/4, 500, 50, 0, 0);   
         ...
         //int cols = rectangles.cols();  
          int rows = rectangles.rows();  
          int elemSize = (int)rectangles.elemSize(); // Returns 12 (3 * 4bytes in a float)  
          float[] data2 = new float[rows * elemSize/4];  
          if (data2.length>0){  
            rectangles.get(0, 0, data2); // Points to the first element and reads the whole thing  
                          // into data2  
